 ...
 * filla-----
 * 
 * Traer un metodo que imprima la metriz de tipo entero
 * (con 2 for)
 * 
 * Traer un metodo
 * que haga el llenado de la matriz por columnas
 * 
 * Traer la suma de valores de la fila
 * en un nuevo arreglo
 * 
 * Traer la suma de columnas
 * 
 * Traer en una matriz cuadrada, sumar la traza (diagonal)
 * */
...
public class ArreglosBidim
{
	public static void main(String[] args) {
		...
		int traza = calcularTraza(matriz);
	...
	public static int[][] llenarMatrizDobleRand (int numFilas, int numColumnas){
	    ...
	    for(int i=0; i<numFilas; i++){
	        for(int j=0; j<numColumnas; j++)
	            matrizLlena[i][j] = (int)(((Math.random())*9));
	    }
	    return matrizLlena;
	}
